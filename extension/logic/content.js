...
class HTMLPersistenceManager {
    ...
    async saveCurrentHTML() {
        try {
            ...
            console.log('HTML changes saved to storage:', this.storageKey);
        } catch (error) {
        ...
        try {
            ...
            if (data && data.html) {
                ...
                if (Date.now() - data.timestamp < maxAge) {
                    console.log('Restoring cached HTML changes for:', this.storageKey);
                    ...
                    console.log('HTML restored from cache');
                    return true;
                } else {
                    console.log('Cached HTML is too old, removing from storage');
    ...
    async clearCache() {
        try {
            ...
            console.log('Cleared cached HTML for:', this.storageKey);
        } catch (error) {
    ...
    async cleanupOldCache() {
        try {
            ...
            if (keysToRemove.length > 0) {
                ...
                console.log('Cleaned up old cached HTML:', keysToRemove.length, 'entries');
            }
        } catch (error) {
...
class AgentObserver {
    ...
    async startObservation() {
        ...
        try {
            ...
            console.log('Sending observation via WebSocket');
            if (window.sendObservation) {
                window.sendObservation(observation);
            } else {
                // --- fallback: send message to popup ---
...
(async function initializePersistence() {
    try {
        ...
        await persistenceManager.restoreHTML();

        console.log('HTML persistence initialized');
    } catch (error) {
...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
    ...
    if (request.action === 'applyAgentEdit') {
        try {
            ...
            // Submit error observation via WebSocket
